...
class Counter extends PureComponent {
  ...
  allImagesTitlesHandler = () => {
    return this.state.updatedImagesDataObj
      ? Object.keys(this.state.updatedImagesDataObj).join('", "')
      : '';
  };
    ...
    switch (elementColor) {
      case 'green':
        for (let imageTitle in this.state.updatedImagesDataObj) {
          if (
            this.state.updatedImagesDataObj[imageTitle].isClickedGreen === true
          ) {
            amountOfElement++;
          }
        }
      ...
      case 'blue':
        for (let imageTitle in this.state.updatedImagesDataObj) {
          if (
            this.state.updatedImagesDataObj[imageTitle].isClickedBlue === true
          )
            amountOfElement++;
        }
        return amountOfElement;
      ...
      case 'red':
        for (let imageTitle in this.state.updatedImagesDataObj) {
          if (
            (this.state.updatedImagesDataObj[imageTitle].isClickedRed ===
              false ||
              this.state.updatedImagesDataObj[imageTitle].isClickedRed ===
                true) &&
            this.state.updatedImagesDataObj[imageTitle].isClickedGreen ===
              false &&
      ...
      case 'orange':
        for (let imageTitle in this.state.updatedImagesDataObj) {
          if (this.state.updatedImagesDataObj[imageTitle].comment !== '')
            amountOfElement++;
        }
        return amountOfElement;
  ...
  allImagesCost = () => {
    ...
    const picturePrice = this.props.picturePrice;
    const discount =
      (amountAll - freePicturesAmount) *
      picturePrice *
      (this.props.discountProcent / 100);
    let price = Math.round(
      (amountAll - freePicturesAmount) * picturePrice - discount,
    );
    if (price < 0) price = 0;
    return price;
  };
    ...
    const filter = buttonType => {
      return (
        <Filter
          ...
          filterButtonsState={filterButtonsState}
        />
      );
    };
      ...
      paymentOnImagesSelect = (
        <React.Fragment>
          <Paper className={classes.paper}>
            ...
            >
              <div
                className={[classes.iconCaptionContainer, classes.green].join(
                  ' ',
                )}
              >
                <div>
            ...
            >
              <div
                className={[classes.iconCaptionContainer, classes.blue].join(
                  ' ',
                )}
              >
                <div>
            ...
            >
              <div
                className={[classes.iconCaptionContainer, classes.red].join(
                  ' ',
                )}
              >
                <div>
            ...
            >
              ...
              >
                ...
                <Avatar
                  className={[classes.avatar, classes.bgOrange].join(' ')}
                >
                  <Typography className={classes.white} variant="caption">
          ...
          <Payment
            countPayPerImage={this.countPayPerImage(
              this.selectedElementsAmount('green'),
            )}
