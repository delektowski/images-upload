...
const paymentConf = ({
  ...
  let discount =
    (picturesAmount - freePicturesAmount) *
    picturePrice *
    (discountProcent / 100);
  let price = Math.round(
    (picturesAmount - freePicturesAmount) * picturePrice - discount,
  );
  if (price < 0) price = 0;
  return (
    <React.Fragment>
      <Fade in={true} timeout={500}>
        <AppBar
          position="static"
          className={isUserCreated ? classes.bar__onUserCreated : classes.bar}
        >
